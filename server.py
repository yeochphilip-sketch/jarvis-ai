...
HEALTH_DIR = _os.path.expanduser('~/jarvis/data/health')
WORKOUT_DIR = _os.path.expanduser('~/jarvis/data/workouts')

@app.route('/health/today', methods=['GET'])
def health_today():
    """Return today's Garmin health data."""
    today = _datetime.now().strftime('%Y-%m-%d')
    health_path = _os.path.join(HEALTH_DIR, f'{today}.json')

    if not _os.path.exists(health_path):
...
def workout_today():
    ...
    today = _datetime.now().strftime('%Y-%m-%d')
    workout_path = _os.path.join(WORKOUT_DIR, f'{today}.json')

    if not _os.path.exists(workout_path):
...
def health_refresh():
    ...
    try:
        result_fetch = subprocess.run(
            ['python3', _os.path.expanduser('~/jarvis/garmin_fetcher.py')],
            capture_output=True, text=True, timeout=60
        )
        result_workout = subprocess.run(
            ['python3', _os.path.expanduser('~/jarvis/workout_coach.py')],
...
@app.route('/health/<date>', methods=['GET'])
def health_by_date(date):
    path = f"data/health/{date}.json"
    if not os.path.exists(path):
        return jsonify({"error": "No data"}), 404
    with open(path) as f:
        return jsonify(json.load(f))

@app.route('/health/weekly', methods=['GET'])
def health_weekly():
