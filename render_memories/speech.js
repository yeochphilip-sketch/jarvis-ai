...
// speak() — called after every Jarvis reply.
// Strips markdown characters, splits into sentences, pre-fetches audio buffers,
// and plays them in sequence with a brief pause between each.
//
...
export async function speak(text) {
  stopSpeaking(); // cancel any in-progress speech first
  ...
  for (let i = 0; i < sentences.length; i++) {
    // [AMENDMENT D — key fix] Check isSpeaking (set false by stopSpeaking()),
    // NOT the orb CSS class (which changes between sentences and broke the loop).
    if (!STATE.isSpeaking) break;

    // Kick off the next fetch while this sentence plays — by the time playback
    // finishes, the next buffer is (usually) already decoded in memory.
    ...
    // Brief pause between sentences. isSpeaking remains true so the loop continues.
  ...
  // If we ran to the end naturally (not interrupted), clean up.
